...
# 15 seconds
# 4, 8, 12, 16, 20 RRHs
# Traffic generation: [614Mb-24.3Gb]/ [expovariate lambda 0.1] s
...
for s in seeds:
    ...
    for f in range(onu_step,max_onus+1, onu_step):
        ...
        matrix = []
        for z in range(f):
            matrix.append([z, f+z, random.randint(5000,9990)])
            matrix.append([f+z, 2*f+1, random.randint(5000,9990)])
        matrix.append([2*f+1, 2*f, 10])
...
plt.subplots_adjust(hspace=0.320)
